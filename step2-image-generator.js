...
async function generateImages() {
  const pool = Pool(
    () => spawn(new Worker("./scripts/generate-image.worker")),
    config.imagesInParallel
  );

  for (let i = 0; i < faces.length; i++) {
    const face = faces[i];
    if (!fs.existsSync(`${config.outputFolder}/images`)) {
      fs.mkdirSync(`${config.outputFolder}/images`);
    }
    const output = `${config.outputFolder}/images/${config.imagePrefix}${
      i + 1
    }.png`;
...
async function main() {
  faces = JSON.parse(
    fs.readFileSync(`${config.outputFolder}/${config.outputJSON}`)
  );
  console.time("images");
